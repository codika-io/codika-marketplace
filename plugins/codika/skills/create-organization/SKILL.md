...
| `--n8n-base-url <url>` | Self-hosted n8n instance URL (admin only, must be paired with `--n8n-api-key`) |
| `--n8n-api-key <key>` | Self-hosted n8n API key (admin only, must be paired with `--n8n-base-url`) |
...
**With self-hosted n8n (admin only):**
...
| "Only administrators can configure self-hosted n8n" | Non-admin tried self-hosted n8n | Only platform admins can set custom n8n instances |
